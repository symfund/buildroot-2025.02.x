...
NUWRITER_TARGET=${BINARIES_DIR}/nuwriter

IMAGE_BASENAME="core-image-buildroot"
MACHINE=
EXT2_SIZE=
ENVOPT="-s 0x10000"
SDCARD=${BINARIES_DIR}/${IMAGE_BASENAME}-${MACHINE}.rootfs.sdcard
...
boot_space()
{
	echo $(sed -n -e 's/^BR2_TARGET_MA35H0_BOOT_SPACE=/ /p' ${BR2_CONFIG} | sed 's/M/ /g' | sed 's/\"/ /g' | sed '/^$/d')
}
...
IMAGE_CMD_spinand() {
	...
	if [ -f ${BINARIES_DIR}/rootfs.ubi ]; then
		if grep -Eq "^BR2_TARGET_MA35H0_SECURE_BOOT=y$" ${BR2_CONFIG}; then
		( \
			cd ${BINARIES_DIR}; \
...
IMAGE_CMD_sdcard() 
{
	...
	if grep -Eq "^BR2_TARGET_MA35H0_SECURE_BOOT=y$" ${BR2_CONFIG}; then
		SDCARD=${BINARIES_DIR}/${IMAGE_BASENAME}-${MACHINE}-enc.rootfs.sdcard
	fi
	
        # Initialize a sparse file
        dd if=/dev/zero of=${SDCARD} bs=1 count=0 seek=$((1024*$SDCARD_SIZE)) &>${NULLDEV}
	/usr/sbin/parted ${SDCARD} -s mklabel msdos
	/usr/sbin/parted ${SDCARD} -s unit KiB mkpart primary \
		$(($BOOT_SPACE_ALIGNED)) \
        	$(($BOOT_SPACE_ALIGNED+$IMAGE_ROOTFS_ALIGNMENT+$EXT2_SIZE))
	/usr/sbin/parted ${SDCARD} print
...
main()
{
	...
	if [ -d ${NUWRITER_TARGET} ]; then rm -rf ${NUWRITER_TARGET}; fi
	mkdir ${NUWRITER_TARGET}

	if [ -f ${BINARIES_DIR}/MBR.sdcard.bin ]; then
		rm ${BINARIES_DIR}/MBR.sdcard.bin
	fi

	if grep -Eq "^BR2_TARGET_MA35H0_SECURE_BOOT=y$" ${BR2_CONFIG}; then
		ECDSA_KEY=$(sed -n -e 's/^BR2_TARGET_MA35H0_ECDSA_KEY=//p' ${BR2_CONFIG} | sed 's/\"//g')
		AES_KEY=$(sed -n -e 's/^BR2_TARGET_MA35H0_AES_KEY=//p' ${BR2_CONFIG} | sed 's/\"//g')
	fi
